...
class RCPurchaseController extends PurchaseController {
  Future<String> getRevenueCatAPIKey() async {
    ...
    if (apiKey == null) {
      throw Exception('REVENUECAT_API_KEY is not set');
    }
    return apiKey;
  }
  ...
  Future<PurchaseResult> purchaseFromGooglePlay(
    ...
    // Try to find the first product where the googleProduct's basePlanId
    // matches the given basePlanId.
    ...
    // If a matching product is not found, then try to get the first
    // product from the list.
    final storeProduct =
        matchingProduct ?? (products.isNotEmpty ? products.first : null);

    // If no product is found (either matching or the first one)
    ...
    switch (storeProduct.productCategory) {
      ...
      case ProductCategory.nonSubscription:
        return _purchaseStoreProduct(storeProduct);
      case null:
  ...
  Future<SubscriptionOption?> _fetchGooglePlaySubscriptionOption(
    ...
    if (subscriptionOptions != null && subscriptionOptions.isNotEmpty) {
      ...
      // Find first subscription option that matches the subscription
      // option ID or use the default offer
      ...
      return subscriptionOption ??= storeProduct.defaultOption;
    }
  ...
  /// Makes a restore with RevenueCat and returns `.restored`,
  ///  unless an error is thrown.
  /// This gets called when someone tries to restore purchases
  ///  on one of your paywalls.
  @override
